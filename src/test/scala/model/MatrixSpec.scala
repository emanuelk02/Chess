    ...
    "A Matrix" when {
        "empty" should {
            ...
            "be instantiated with a full Matrix given as a Vector of Vectors" in {
                ...
                matr.cell("A", 0).get should be(W_PAWN)
                matr.cell("B", 0).get should be(B_KING)
            }
        ...
        "filled" should {
            ...
            "return contents from single cells" in {
                ...
                matr.cell("A", 0) should be(Some(W_BISHOP))
                matr.cell("B", 0) should be(Some(W_BISHOP))
            ...
            "not return a diferent size based on contents" in {
                ...
                matr.replace("B", 1, Some(B_KING)).size should be(matr.size)
            ...
            "allow to replace single cells at any location and return the new matrix" in { 
                ...
                matr.replace("A", 0, Some(B_KING)) should be(Matrix(Vector(Vector(Some(B_KING), Some(W_BISHOP)), Vector(Some(W_BISHOP), Some(W_BISHOP)))))
                matr.replace("B", 1, Some(B_KING)) should be(Matrix(Vector(Vector(Some(W_BISHOP), Some(W_BISHOP)), Vector(Some(W_BISHOP), Some(B_KING)))))
            }
