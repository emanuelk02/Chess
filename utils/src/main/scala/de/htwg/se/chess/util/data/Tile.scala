...
object Tile:
    ...
    def apply(args: List[Char], size: Int): Tile = Tile(args(0).toLower.toInt - 'a'.toInt + 1, (args(1).toInt - '0'.toInt), size)
    def apply(args: List[Char]): Tile = Tile(args(0).toLower.toInt - 'a'.toInt + 1, (args(1).toInt - '0'.toInt))
    ...
    def withRowCol(row: Int, col: Int, size: Int) = Tile(col + 1, size - row, size)
