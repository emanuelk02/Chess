...
case class MatrixWrapper(field: Matrix[Option[Piece]], state: ChessState):
    ...
    private lazy val allTiles: Seq[Tile] =
        (1 to size)
        .flatMap( file => (1 to size)
            .map( rank => Tile(file, rank, size) )
            )

    def filterOutIllegalMoves(moves: Seq[(Tile, List[Tile])]): Seq[(Tile, List[Tile])] =
      moves
    ...
    lazy val legalMoves: Map[Tile, List[Tile]] =
      Map from
        filterOutIllegalMoves(
          allTiles.map(tile => tile -> computeLegalMoves(tile))
        )

    def getAllLegalMoves: Map[Tile, List[Tile]] = legalMoves

    def getLegalMoves(tile: Tile): List[Tile] =
        filterOutIllegalMoves(
          List(tile -> computeLegalMoves(tile))
        ).headOption match
            case Some((_, moves)) => moves
            case None => Nil

    lazy val getKingSquare: Option[Tile] =
    ...
    private val inverseColorTileHandle = ChainHandler[Tile, Tile] (List[Tile => Option[Tile]]
      (
        ( tile => if cell(tile).isDefined then None else Some(tile) ),
        ( tile => if cell(tile).get.getColor == state.color then Some(tile) else None )
      )
    ...
    private def inverseColorRookMoveChain = inverseColorSlidingMoveChain(straightMoves) _
    ...
    lazy val inCheck = getKingSquare match
...
object MatrixWrapper:
    ...
    def apply(field: Matrix[Option[Piece]], state: ChessState): MatrixWrapper =
        prototype.copy(field = field, state = state)